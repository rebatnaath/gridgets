...
let citiesRemoteFetchInFlight = false;
let cachedDesktopApplications = null;

const REMOTE_CITIES_DATABASE_URL = 'https://raw.githubusercontent.com/rebatnaath/gridgets/main/assets/datas/cities.json';
...
function fetchRemoteCitiesDatabase() {
    ...
    citiesRemoteFetchInFlight = new Promise((resolve, reject) => {
        ...
        session.send_and_read_async(message, GLib.PRIORITY_DEFAULT, null, (s, res) => {
            try {
                ...
                if (Array.isArray(data) && data.length > 0) {
                    ...
                    console.log(`Gridgets: Loaded remote cities database (${data.length} entries)`);
                    resolve(data);
                    return;
                }
...
export function openAddWorldClockDialog(parentWindow, settings) {
    ...
    const primaryPicker = createLiveCitySearchRow(grid, 'Primary City (Top):', { name: 'London', timezone: 'Europe/London' }, 0);
    const sec1Picker = createLiveCitySearchRow(grid, 'Secondary City (Bottom Left):', { name: 'New York', timezone: 'America/New_York' }, 1);
    const sec2Picker = createLiveCitySearchRow(grid, 'Secondary City (Bottom Right):', { name: 'Moscow', timezone: 'Europe/Moscow' }, 2);
