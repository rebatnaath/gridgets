...
import { createWidgetContainer, drawCircularArc } from '../../utils/widgetUIUtils.js';
...
export function createCpuRamNode(config, width, height, xPosition, yPosition) {
    ...
    cpuSection.canvas.connect('repaint', (area) => {
        ...
        drawCircularArc(ctx, canvasWidth, canvasHeight, currentCpuProgress, DEFAULT_CPU_COLOR, undefined, textColor);
    });
    ...
    ramSection.canvas.connect('repaint', (area) => {
        const ctx = area.get_context();
        const [canvasWidth, canvasHeight] = area.get_surface_size();
        drawCircularArc(ctx, canvasWidth, canvasHeight, currentRamProgress, DEFAULT_RAM_COLOR, undefined, textColor);
    });
